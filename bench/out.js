...
if ((typeof performance === 'undefined' ? 'undefined' : _typeof(performance)) === 'object' && typeof performance.now === 'function') {
  now$1 = performance.now.bind(performance);
  format = 'milli';
} else if (typeof process !== 'undefined' && typeof process.hrtime === 'function') {
  now$1 = process.hrtime.bind(process);
  format = 'hrtime';
} else {
  ORIGIN_TIME = Date.now();
  now$1 = Date.now.bind(Date);
  format = 'timestamp';
}
  ...
  createClass(Heimdall, [{
    ...
    value: function node(name, Schema, callback, context) {
      ...
      return new Promise$1(function (resolve) {
        return resolve(callback.call(context, node.stats.own));
      }).finally(function () {
        return _this.stop(token);
      });
    }
  }, {
    key: 'registerMonitor',
