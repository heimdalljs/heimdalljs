...
module.exports = function contextFor({
  ...
  let contextRows = node.nodes.map((child) => {
    ...
    let rv = [
      `${prefix(depth)}${child.name}`,
      ...stats.map(getStat(child, ignoreBranches)),
    ];
    rv.node = child;
    return rv;
  });

  contextRows = contextRows.filter(function (v) {
    return v !== undefined;
  });

  contextRows.sort((a, b) => b[sortByCol] - a[sortByCol]);

  contextRows = compressContext({
    contextRows,
    depth,
    ignoreBranches,
    compressAfter,
